...
if __name__ == '__main__':
    ...
    rospy.init_node('robot_b_teleoperation_node')

    # Subscribe to the topics and associate the corresponding callback functions
    sub_qwe = rospy.Subscriber('/robot_b/teleoperation/key_qwe/', Vector3, qwe_callback)
    sub_asd = rospy.Subscriber('/robot_b/teleoperation/key_asd/', Vector3, asd_callback)
    sub_pose = rospy.Subscriber('/robot_b/teleoperation/mouse_pose/', Vector3, pose_callback)

    # Publish messages
    pub_vel = rospy.Publisher('/robot_b/cmd_vel/', Twist, queue_size=10)
    pub_pan = rospy.Publisher('/robot_b/joint_pan_position_controller/command', Float64, queue_size = 1)
    pub_tilt = rospy.Publisher('/robot_b/joint_tilt_position_controller/command', Float64, queue_size = 1)
